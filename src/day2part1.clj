...
(defn split-by [pattern str] (string/split str pattern))
...
(defn parse-game [game-str]
  (let [sections (split-by #":" game-str)
        rounds (->>
                sections
                last
                (split-by #";")
                       ...
                       (->> round-str
                            ...
                            (map (fn [count-str]
                                   (->>
                                    ...
                                    string/trim
                                    (split-by #" ")
                                    ((fn [[num color]] [color, (parse-long num)])))))
                            (into {})))))]
    {:id (->>
          sections
          first
...
(defn game-fits-contraints [game]
  (->>
   (get game :rounds)
   (every? (fn [round]
             (->>
              constraints
              (every? (fn [[constraint-color constraint-count]]
                        (or
                         (nil? (get round constraint-color))
                         (<= (get round constraint-color) constraint-count)))))))))

(def input "Game 1: 9 red, 2 green, 13 blue; 10 blue, 2 green, 13 red; 8 blue, 3 red, 6 green; 5 green, 2 red, 1 blue
...
(defn total-value [input]
  (->>
   ...
   string/split-lines
   (map parse-game)
   (filter game-fits-contraints)
   (map (fn [game] (get game :id)))
   (reduce +)))

(deftest day-2-part-1
  (is (= 2283 (total-value input))))
